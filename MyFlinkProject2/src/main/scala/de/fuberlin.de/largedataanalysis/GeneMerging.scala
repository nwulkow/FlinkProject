...
object GeneMerging {
  ...
  def main(args: Array[String]) {
    ...
    //val iddata = getIDDataSet(env, firstDatasetPath)
    //val genedata = getGeneDataSet(env, secondDatasetPath)
    val genedatafull = getGeneDataFullSet(env, firstDatasetPath)
    val stagedatafull = getStageDataFullSet(env, secondDatasetPath)
    ...
    mlr.fit(lvset)
    println("HIER SIND WIR JETZT VOR DEN GEWICHTEN!!")
    print(mlr.weightsOption)
    println("NACH DEN GEWICHTEN!!")*/
  ...
  /*case class Data(Id: String, CancerStage: String, Gene1Count: String, Gene2Count: String)
  case class IdData(Id: String, CancerStage: String)
  case class GeneData(Id: String, Gene1Count: String, Gene2Count: String)
  */
 ...
 /* private def getIDDataSet(env: ExecutionEnvironment, path: String): DataSet[IdData] = {
    env.readCsvFile[IdData](
      path,
      fieldDelimiter = ",")
  }

  private def getGeneDataSet(env: ExecutionEnvironment, path: String): DataSet[GeneData] = {
